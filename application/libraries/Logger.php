...
/**
 * @package Logger
 * @author Talha <y2jsheikh@hotmail.com>
 * @version V1.0
...
class Logger {
    ...
    public function __construct() {
        $CI = & get_instance();
        $CI->load->config('logger', TRUE);
        $CI->load->model('logger_model');
        $this->_store_in = $CI->config->item('store_in', 'logger');
    ...
    //    $session_user_id = $_SESSION['logged_in']['id'];
        if (($user_id == 0) && isset($_SESSION['logged_in']['id'])) {
            $user_id = $_SESSION['logged_in']['id'] > 0 ? $_SESSION['logged_in']['id'] : '0';
        }
/*
        $session_user_id = $CI->config->item('session_user_id', 'logger');
...
*/
    ...
    private function _set_transaction($transaction_details, $user_id) {
        ...
        if ($detect->isMobile()) {
            ...
            $user_agent = $this->get_user_agent($browser);
        }
        ...
        if ($this->_store_in == 'database') {
            if ($CI->logger_model->set_transaction($transaction)) {
                return TRUE;
            }
        }
        return FALSE;
    ...
    private function _get_transaction($user_id = NULL, $date = NULL, $order_by = NULL, $limit = NULL) {
        ...
        if ($this->_store_in == 'database') {
            ...
            return $CI->logger_model->get_transaction($where, $order_by, $limit);
        }
    }
