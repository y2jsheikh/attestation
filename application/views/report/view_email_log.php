                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="email" id="to" class="form-control input-paf" placeholder="Enter Email" />
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label class="control-label">Sent Status: </label>
                            <select id="send_status" class="form-control input-paf select2">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <button id="search-btn" type="button" class="btn btn-primary btn-sm"><i class="fa fa-search"></i> Search</button>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.send-again-btn', function () {
            $.ajax({
                url: '<?php echo base_url('ajax/resend_email'); ?>',
                data: {"email_log_id": $(this).val()},
                type: 'POST',
                success: function (data) {
                    var response_value = $.parseJSON(data);
                    if (response_value == 'sent') {
                        alert("Email Sent");
                        show_ajax_cards('');
                    } else if (response_value == 'not_sent'){
                        alert("Email Not Sent");
                        return false;
                    } else if (response_value == 'localhost'){
                        alert("Email cannot be sent on Localhost");
                        return false;
                    }
                }
            });
